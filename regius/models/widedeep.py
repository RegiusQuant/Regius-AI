# -*- coding: utf-8 -*-
# @Time       : 2020/10/18 18:06:13
# @Author     : RegiusQuant <315135833@qq.com>
# @Project    : Regius-AI
# @File       : widedeep.py
# @Description: 深度学习WideDeep模型
...
from regius.data import WideDeepDataset
...
class WideDeepModel(pl.LightningModule):

    def __init__(self, wide_num, cont_num, cate_input):
        super().__init__()
        self.model = WideDeep(wide_num, cont_num, cate_input)

    def forward(self, x_wide, x_cont, x_cate):
        out = self.model(x_wide, x_cont, x_cate)
        return nn.Sigmoid()(out) * ARGS["sigmoid_coef"]

    def training_step(self, batch, batch_idx):
        x_wide, x_cont, x_cate, y_true = batch[0].float(), batch[1].float(), batch[2].long(), batch[3].float()
        y_pred = self(x_wide, x_cont, x_cate).squeeze()
        loss = F.mse_loss(y_pred, y_true)
        self.log("train_loss", loss)
        return loss

    def test_step(self, batch, batch_idx):
        x_wide, x_cont, x_cate, _ = batch[0].float(), batch[1].float(), batch[2].long(), batch[3].float()
        y_pred = self(x_wide, x_cont, x_cate).squeeze()
        return y_pred
...
def run_cross_validation(file_path_list, pkl_path):
    ...
    for i in range(len(file_path_list)):
        ...
        wide_cols = train_data.filter(regex="C_*").columns
        x_wide_train, x_wide_valid = train_data[wide_cols].copy(), valid_data[wide_cols].copy()
        encoder = OneHotEncoder(sparse=False)
        x_wide_train = encoder.fit_transform(x_wide_train)
        x_wide_valid = encoder.transform(x_wide_valid)
        wide_num = x_wide_train.shape[1]
        ...
        train_set = WideDeepDataset(x_wide_train, x_cont_train, x_cate_train, y_train)
        valid_set = WideDeepDataset(x_wide_valid, x_cont_valid, x_cate_valid, y_valid)
        ...
        model = WideDeepModel(wide_num, cont_num, cate_input)
...
def run_final_predict(train_file_path_list, test_file_path, pkl_path, folder_name):
    ...
    wide_cols = train_data.filter(regex="C_*").columns
    x_wide_train, x_wide_test = train_data[wide_cols].copy(), test_data[wide_cols].copy()
    encoder = OneHotEncoder(sparse=False)
    x_wide_train = encoder.fit_transform(x_wide_train)
    x_wide_test = encoder.transform(x_wide_test)
    wide_num = x_wide_train.shape[1]
    ...
    train_set = WideDeepDataset(x_wide_train, x_cont_train, x_cate_train, y_train)
    test_set = WideDeepDataset(x_wide_test, x_cont_test, x_cate_test, y_test)
    train_loader = DataLoader(train_set, batch_size=ARGS["batch_size"], num_workers=4, shuffle=True)
    ...
    trainer = pl.Trainer(max_epochs=ARGS["max_epochs"], gpus=1)
    trainer.fit(model, train_loader)
    model_path = f"model/{folder_name}/widedeep.ckpt"
    trainer.save_checkpoint(model_path)

    model = WideDeepModel.load_from_checkpoint(
    ...
    col_name = "WIDEDEEP_RESULT"
    test_data[col_name] = y_pred
    _, name = os.path.split(test_file_path)
    result_path = f"result/{folder_name}/model_widedeep_{name}"
...
def run_random_shuffle_test(file_path_list, pkl_path, folder_name):
    ...
    result_path = f"result/{folder_name}/model_widedeep_randomshuffle.csv"
